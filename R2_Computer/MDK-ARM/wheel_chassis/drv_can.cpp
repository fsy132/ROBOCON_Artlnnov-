...
{
    static int mod10 = 0;
    static uint8_t side_tx_div = 0;
    static uint8_t can2_tx_phase = 0;

    mod10++;

    // Keep CAN1 chassis ID1-4 current frame at 1 kHz.
    CAN_Send_Data(&hcan1, 0x200, CAN1_0x200_Tx_Data, 8);

    // Send CAN1 ID5-8 frames at 500 Hz to reduce CAN1 load; side wheels need less realtime than chassis.
    side_tx_div ^= 1U;
    if (side_tx_div == 0U)
    {
    ...
    // Stagger CAN2 motor current frames at 500 Hz each; 7 motors already make CAN2 very busy.
    can2_tx_phase ^= 1U;
    if (can2_tx_phase == 0U)
    {
